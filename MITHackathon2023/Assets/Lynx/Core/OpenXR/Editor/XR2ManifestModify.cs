    ...
    internal class XR2ManifestModify : OpenXRFeatureBuildHooks
    {
        ...
        private class AndroidManifest : AndroidXmlDocument
        {
            ...
            private void AddUsesPermission(string permission)
            {
                ...
                if (!(from XmlNode node in xmlNodeList from XmlAttribute attrib in node.Attributes select attrib).Any(attrib => attrib.Name == "name" && attrib.Value == permission))
                {
            ...
            private void AddUsesFeature(string feature, bool required)
            {
                ...
                // Create node only if the tag does not exist
                if (!(from XmlNode node in xmlNodeList from XmlAttribute attrib in node.Attributes select attrib).Any(attrib => attrib.Name == "name" && attrib.Value == feature))
                {
            ...
            private void AddIntentAction(string actionName)
            {
                ...
                if (!(from XmlNode node in xmlNodeList from XmlAttribute attrib in node.Attributes select attrib).Any(attrib => attrib.Name == "name" && attrib.Value == actionName))
                {
            ...
            private void AddAndroidPackage(string packageName)
            {
                ...
                if (!(from XmlNode node in queriesNode from XmlAttribute attrib in node.Attributes select attrib).Any(attrib => attrib.Name == "name" && attrib.Value == packageName))
                {
